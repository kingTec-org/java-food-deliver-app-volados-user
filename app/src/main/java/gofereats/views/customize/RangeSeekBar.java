...
import gofereats.R;
import gofereats.interfaces.OnRangeSeekBarChangeListener;
import gofereats.interfaces.OnRangeSeekBarFinalValueListener;
...
public class RangeSeekBar extends View {
    ...
    private Thumb evalPressedThumb(float touchX) {
        ...
        if (minThumbPressed && maxThumbPressed) {
            // if both del_car are pressed (they lie on top of each other), choose the one with more room to drag. this avoids "stalling" the del_car in AddCardActivity corner, not being able to drag them apart anymore.
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsDragging) {
                    ...
                    // should be interpreted as AddCardActivity tap-seek to that location.
                    onStartTrackingTouch();
                    trackTouchEvent(event);
                    onStopTrackingTouch();
                }
